...
### 🚨 核心规则：用户说"优化"= 跑 `run_optimization()`

**禁止**手动改参数（如把 EMA20 改成 EMA15、RSI 55 改成 54）然后重新回测。
**禁止**自己分析哪个参数应该改成多少。
**必须**把参数做成模板，让服务器用遗传算法/贝叶斯优化自动搜索最优组合。

用户说以下任何一种，**全部走 `run_optimization`**：
- "优化"、"优化策略"、"优化参数"
- "改进"、"调参"、"提高收益/胜率/Sharpe"
- "批量回测"、"对比参数"、"找最优"
- "再优化一下"、"继续调"

**正确做法**：从当前策略中提取 EMA周期/RSI阈值/ATR倍数等做成 PARAMS → 调 `run_optimization(method="genetic")` → 服务器自动跑 50+ 组合 → 展示 Top 5。
**错误做法**：自己把 EMA20→EMA15、ATR 3.0→3.5 然后回测一次。这不叫优化，叫猜。
